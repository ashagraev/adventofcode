...
public:
    ...
    size_t NumberOfPaths(string start, string finish, std::shared_ptr<TraversalPolicy> policy) const {
        ...
        for (auto&& it : neighbours->second) {
            ...
            policy->PushVisit(it);
            result += this->GeneratePaths(it, finish, policy, currentPath + "," + it);
            policy->PopVisit(it);
        }
        return result;
    }
    ...
    size_t NumberOfPaths() const {
        return NumberOfPaths("start", "end", std::make_shared<Policy>("start"), "start");
    }
};
...
class DefaultPuzzlePolicy: public TraversalPolicy {
private:
    unordered_map<string, int> Counter;
public:
    DefaultPuzzlePolicy(string start) {
        ++Counter[start];
    }
...
int main() {
    ...
    Graph g;
    g.Load(in);
    cout << "default policy paths: " << g.NumberOfPaths<DefaultPuzzlePolicy>() << std::endl;
